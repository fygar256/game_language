...
そのエディタ・インタプリタ・コンパイラシステム（一応IDE)はMIEPといい、今回のGAME言語インタプリタの名前もmiepにしました。MIEPの他に、高校生の頃、MIGというコンパイラも作りました。それはIDEを作ろうと思っていたのですが、途中で断念して、コンパイラシステムだけになりました。

それでも、インラインアセンブラを装備するなどしていました。
